...
OBS: A partir do python versão 3.x a função reduce() não é mais uma função integrada (built-in). Agora temos que
...
# Na prática ?
# Vamos utilizar a função reduce() para multiplicar todos o números de um lista

from functools import reduce
dados = [2, 3, 4, 5, 7, 11, 13, 17, 19, 23, 29]

# Para utilizar o reduce() nós precisamos de uma função que receba dois parâmetros
# OBS: TypeError se você passar parâmetros a mais ou a menos
multi = lambda x, y: x * y
res = reduce(multi, dados)
print(res)

# Utilizando um loop norma
res = 1
for n in dados:
    res = res * n
print(res)
